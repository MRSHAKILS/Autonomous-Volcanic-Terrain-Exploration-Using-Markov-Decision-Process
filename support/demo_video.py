...
the real pipeline (``python main.py``), the generated mission map opens, the
baseline comparison runs (``python support/experiments.py``), and the
performance plot opens. All terminal text is the genuine output of those
commands, captured when this script runs, so the video always matches the
current code.
...
def _typing_frames(previous_lines: list[str], command: str, seconds: float) -> list[dict]:
    ...
    for index in range(count):
        ...
        frames.append({"lines": previous_lines + [PROMPT + typed], "image": None})
...
def _output_frames(previous_lines: list[str], command: str, output: list[str], seconds: float) -> list[dict]:
    ...
    for index in range(count):
        ...
        frames.append({"lines": base + output[:shown], "image": None})
...
def _image_frames(lines: list[str], image_path: Path, title: str, seconds: float) -> list[dict]:
    ...
    count = max(int(seconds * FRAMES_PER_SECOND), 1)
    return [{"lines": lines, "image": (image_path, title)} for _ in range(count)]
...
def build_frames() -> list[dict]:
    ...
    # 2-26s: the full pipeline output scrolls through.
    frames += _output_frames([], "python main.py", main_output, 24)
    # 26-34s: the generated mission map opens.
    tail_after_main = frames[-1]["lines"]
    frames += _image_frames(tail_after_main, mission_map, "outputs/final_map.png — mission map", 8)
    # 34-36s: type the experiments command.
    frames += _typing_frames(tail_after_main + [""], "python support/experiments.py", 2)
    # 36-47s: the comparison summary appears.
    frames += _output_frames(tail_after_main + [""], "python support/experiments.py", experiments_tail, 11)
    # 47-56s: the performance plot opens.
    tail_after_experiments = frames[-1]["lines"]
    frames += _image_frames(tail_after_experiments, performance_plot, "outputs/performance_plot.png — MDP vs baselines", 9)
    # 56-60s: closing terminal state.
    closing = tail_after_experiments + ["", PROMPT + "# every run is seeded and fully reproducible"]
    frames += [{"lines": closing, "image": None}] * (4 * FRAMES_PER_SECOND)
